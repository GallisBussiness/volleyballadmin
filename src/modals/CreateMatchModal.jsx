...
const CreateMatchModal = ({ isOpen, onResolve, onReject }) => {

  const [tournois,setTournois] = useState([]);
  const [equipes,setEquipes] = useState([]);
  const [curTournoi,setCurTournoi] = useState(null);
  const [curTid,setCurTid] = useState(null);
  ...
  useQuery(qke, () => getEquipeByTournoi(curTid), {
   ...
   onSuccess: (_) => {
    ...
    const defaultValues = {tournoi: '',date: new Date().toISOString(), heure: new Date(),lieu: '',equipeA: '', equipeB:''};
  ...
  return (
    <>
       <Dialog header="Creer un Match" visible={isOpen} onHide={() => onReject()} className="w-1/2">
    ...
    <div className="mb-3">
           ...
           {curTournoi && <>
            ...
            <Button type="submit" className="bg-yellow-500 hover:bg-yellow-700"> CREER LE MATCH</Button>
          </form>
  </Dialog>
    </>
  )
...
export default create(CreateMatchModal);